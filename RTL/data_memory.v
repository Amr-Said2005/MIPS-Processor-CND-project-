module data_memory (
    ...
    always @(posedge clk or negedge rst_a or negedge rst_r) begin
        ...
        else if (memWrite) begin                // store (little-endian)
    ...
    // combinational read: single-cycle needs the data in the same cycle
    assign data_out = memRead ? { mem[byte_addr + 3], mem[byte_addr + 2],
